...
namespace ACFUN
{
    public partial class MentionsPage : PhoneApplicationPage, INotifyPropertyChanged
    {
        ...
        private void EasyListBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            {
                var item = lb.SelectedItem as Mention;
        ...
        private async Task getList()
        {
            ...
            try
            {
                using (var stream = await new HttpHelp().Get(string.Format(StaticData.mention, page)))
                {
                    ...
                    var totalPage = (int)obj["totalPage"];
                    if (page < totalPage)
                    {
                        page++;
                    }
                    ...
                    var contentList = new List<ACItem>();
                    ...
                    if (commentList.Count > 0)
                    {
                        for (var i = 0; i < commentList.Count; i++)
                        {
                            try
                            {
                                ...
                                var comment = commentContentArr[commentList[i]];
                                ...
                                {
                                    var pcomment = commentContentArr[comment.quoteId];
